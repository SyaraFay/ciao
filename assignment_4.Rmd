...
author: |
  | Syara Letwory
  | s.f.letwory@students.uu.nl
  | http://www.github.com/SyaraFay
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW$treat <- ifelse(GrowthSW$revolutions > 0, "Revolutions more than 0", "Revolutions equal to 0")
...
**Designated place**: The mean is higher, when there are no revolutions, than when there are more than 0 revolutions. This apply for both variables.
...
```{r}
NoRevolutions <- subset(GrowthSW$growth, GrowthSW$treat == "Revolutions equal to 0")
Revolutions <- subset(GrowthSW$growth, GrowthSW$treat == "Revolutions more than 0")

ttest <- t.test(NoRevolutions, Revolutions)
print(ttest)
```

**Question 3.2**: What is the $p$-value of the test, and what does that mean? Write down your answer below.

The p-value is 0.06871, this means that the observed data will occur about 6.871% of the times.
...
`rgdp60` is the value of the GDP per capita in 1960, converted to 1960 US dollars. I think the purpose to include the variable `rgdp60` is to show that countries with different gdp can have different growth rates. 
...
According to this statistic, the main driver of economic growth is tradeshare, as it has the largest number out of all the variables. 
...
list(model1, model2, model3, model4) |>
  modelsummary(stars=T, gof_map = c("nobs", "r.squared")) |>
   kableExtra::row_spec(3:4, background = 'red', color = 'white')
...
```{r}
doc <- read_docx() %>%
  body_add_flextable(value = table_output)
